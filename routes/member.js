...
router.get("/blog/:aId?", function(req, res) {
  ...
  db.queryAsync(sql, [req.params.aId]).then(result => {
    return res.json(result);
  });
});
...
router.post("/Sorder/update/:orderId?", function(req, res) {
  ...
  db.queryAsync(sql, [req.body.orderStsId, req.params.orderId]).then(result => {
...
router.get("/comment/:mId?", function(req, res) {
  const sql = `SELECT service_comment.sId as 'id',service_comment.mId as 'mId',service_comment.commentTxt as 'txt',service_comment.rating as 'rating',service_user.sName as 'name' FROM service_comment INNER JOIN service_user WHERE service_comment.sId = service_user.id AND service_comment.mId= ?`;
  // console.log(req.params.mId);
  db.queryAsync(sql, [req.params.mId]).then(result => {
    return res.json(result);
  });
